...
Article-specific analysis artifacts:

- `article_analysis/limited_hypothesis_tests.csv` contains the prespecified limited hypothesis tests with FDR-corrected p-values;
- `article_analysis/automated_manipulation_checks.csv` contains the automated manipulation-check proxies;
- `article_analysis/quality_failure_audit.csv` summarizes technical stability and failure counts across the main experiments and ablations;
- `article_analysis/manual_coding_first_pass.csv` provides the first-pass manually reviewable coding sheet used for exploratory inspection.

Targeted ablation artifacts:

- `article_ablation/urgency_ablation_*.csv` and `article_ablation/trust_ablation_*.csv` contain the targeted ablation results, confidence intervals, and auxiliary tests used in the article.
