...
```
Fake-anger negotiation ablation:
```powershell
python run_anger_ablation_experiment.py
```
False-reliability trust ablation:
```powershell
python run_false_reliability_ablation_experiment.py
```
...
The `results/final/` directory contains the final artifacts used in the paper.
Main compact tables:
...
## Ablation Results
The `results/final/ablations/` directory contains the ablation artifacts used in the paper.
Two targeted ablations isolate the contribution of emotional language above the action policy for two mechanism families:
- `anger_ablation/` — fake-anger negotiation ablation (96 games, 24 per condition). Four conditions: `neutral_control`, `fake_anger_bundle`, `anger_language_only`, `anger_policy_only`. Each condition uses six fully counterbalanced model-family pairs (llama3.1:8b, qwen2.5, gemma3:4b).
- `false_reliability_ablation/` — false-reliability trust ablation (96 games, 24 per condition). Four conditions: `neutral_control`, `false_reliability_bundle`, `reliability_language_only`, `opportunistic_policy_only`. Belief tracking is enabled throughout (every 2 rounds), matching the main trust experiment protocol.
Each ablation directory contains a full JSON episode log and a summary CSV with per-condition means.
